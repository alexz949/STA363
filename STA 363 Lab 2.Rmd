...
```{r}
library(readxl)
DataForFigure2.1WHR2023 <- read_excel("DataForFigure21WHR2023.xls")
```
...
# in the happiness25 data set
whichMissing <- which(is.na(happiness25[11,]$happiness) == TRUE)
...
The 95% confidence intervals for the UMI model and full model are different. The new 95%CI for hte intercept decreases while that for the logGDP increases.
THe 95% CI for the UMI model and the model with missing data are the same.
...
compute_RMSE <- function(truth, imputations){
  ...
  #Part 3: RSS 
  part3 <- sum(which(is.na(part2) == TRUE))
...
Because column 1 is about name while column 9 is the duplicated column of happiness we created for the previous question.
```{r}
...
```{r}
# Pull the imputations
imputations_UMI <-  happiness25[whichMissing, "happiness_Regression"]
...
Since we only have 20% of the data observed, we might not use the estimation of the missing data. We may suggest considering another variable to be used in the model.
...
Since there is low relavance, we may also consider eliminate this varible in the model.
